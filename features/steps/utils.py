...
def generate_gln(company_prefix: str) -> str:
    gln = company_prefix + generate_x_length_number(6)
    return gln
...
def generate_sgln_from_gln(gln: str) -> str:
    ...
    serial_number = "0"
    sgln_base = f"{company_prefix}.{location_reference}.{serial_number}"

    # check_digit = calculate_check_digit(f"{company_prefix}{location_reference}{serial_number}")
    # sgln = f"urn:epc:id:sgln:{sgln_base}{check_digit}"
